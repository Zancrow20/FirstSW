...
public class UserRepository : IRepository<User_ID>
{
    ...
    public async Task<User_ID?> GetById(int id)
        => await DB.Select<User_ID>(id);

    public async Task<int> Create(User_ID user)
        => await DB.Insert(user);

    //public async Task<int> Create(string nickname, string password)
    ...
    public async Task<User_ID?> GetEntityByProperties(params object[] properties)
    {
        var email = properties[0].ToString();
        var user = (await DB.Select<User_ID>(email, "email")).FirstOrDefault();
        return user ?? null;
    }
}